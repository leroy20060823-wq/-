...
바로 전달**합니다(문서 → 워드 `.docx` + `PDF`, 발표 → 파워포인트 `.pptx` + `PDF`).
**한글 `.hwpx`** 는 "한글로 줘"처럼 말하면 추가됩니다. 오프라인 Node 변환기
`scripts/export.mjs`가 `outputs/`에 파일을 만듭니다 — 브라우저 내보내기
`public/{docx,hwpx,pptx}.js`와 동일한 문서 매핑을 DOM 대신 `marked` 렉서로 구현하고,
PDF는 Chromium(Playwright) + 번들 한글 폰트(`fonts/`)로 렌더해 한글·발음기호도 안
깨집니다.
