...
2. 출력한 뒤 한 줄로 제안하세요: `파일로 저장할까요? (파워포인트(.pptx) · 마크다운(.md)) 아니면 고칠 부분이 있나요?`
3. **사용자가 파일/저장/인쇄/한글/워드/PPT를 원하거나, 처음부터 형식을 지정했다면** 다음을 수행하세요.
   ...
   2. 아래 명령으로 문서 파일을 만드세요(저장소 루트에서 실행):
      ```bash
      node scripts/export.mjs --in "outputs/<이름>.md" --format pptx --out "outputs/<이름>" --title "<이름>"
      ```
   3. 생성된 파일 경로(파워포인트(.pptx) · 마크다운(.md))를 알려 주세요. 사용자는 내려받아 인쇄·배포할 수 있어요.
