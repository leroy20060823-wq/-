...
const MODULES: GenerationModule[] = [
  {
    ...
    systemPrompt: [
      ...
      "- Otherwise (topic only), invent brand-new content every run. Never copy another exam's content — a reference example shows STRUCTURE and TONE only, never its content.",
      "",
      "Document structure (clean Markdown — follow EXACTLY so it renders):",
      "1. '# <Title>', then a meta line '총 N문항 · 시험시간 M분 · 100점 만점 | 출제 범위: ...', then '난이도: <하|중|상> · <설명>'.",
      "2. '## 수험자 유의사항' followed by a numbered list (답안 표기 방식, 서술형 문항 안내, 시험 시간, 정답·해설 위치).",
      "3. '## 배점표': a Markdown table whose header is EXACTLY '파트 | 파트명 | 유형 | 문항 범위 | 문항 수 | 문항당 배점 | 파트 총점', one row per part (문항 범위 like '1~6'), and a final '합계' row. Points MUST total 100.",
      "4. Parts, each headed '## P1. <name>' … (default 4 parts: P1 어휘·어법, P2 독해, P3 서술형, P4 문법어법; adapt names to the subject).",
      "   - Reading passage: '### <title> — <theme tag>' then its paragraphs, placed right before the items that use it.",
      "   - Item: '**N. <type> [<점수>점]**' then the stem. A focal quoted sentence goes on its OWN line in straight quotes (it renders in a box). Append ' ★Killer' for the hardest items.",
      "   - EVERY 객관식 item has FIVE options, each on its OWN line labelled 'A) ' … 'E) ', with exactly one correct (the renderer shows A–E as ①②③④⑤). 서술형: the prompt, then a blank line '✏️ ____', with no options.",
      "   - Number items continuously across all parts.",
      "5. A closing line, then the answer section.",
      "",
      "Answer section:",
      "- '## 정답표 (Answer Key)' grouped by part, e.g. '**P1.**  1. A  2. C  3. B', using option letters A–E (rendered ①②③④⑤) and ' ★' on killers. 서술형 → 모범 답안/채점 기준.",
      "- '## 정밀 해설지 (Detailed Explanations)' in a warm, friendly '~요' teacher voice. For EACH item give: '**N. 정답 X**' (X = A–E), one reasoning sentence grounded in the passage/rule, a '핵심' line, and an '오답 체크' line explaining why the attractive distractors are wrong. 서술형 → 모범 답안.",
      "",
      "Difficulty rubric — make 하 / 중 / 상 genuinely different (지문 어휘·문장 복잡도, 추론 비율, 함정 선지 정교함, 킬러 수). Apply precisely, not just by wording:",
      ...
      "- 함정 선지: 하 = 명백히 틀린 선지; 중 = 그럴듯한 함정 1개; 상 = 지문 일부만 맞거나 방향·기간·범위만 뒤집은 정교한 함정.",
      ...
      "- 배점표, items, and 정답표 must agree in counts and numbering. EVERY keyed answer must be provable from the printed passage/stem ALONE, with exactly one defensible option per item.",
      "- Follow any '[요청 조건]' (난이도, 시험 시간, 유형별 개수) and keep the total at 100점; reflect the requested 시험 시간 in the meta line.",
      "- Add NO school/brand name or motto unless the request explicitly provides one.",
    ].join("\n"),
    referenceExample: [
      "| 파트 | 파트명 | 유형 | 문항 범위 | 문항 수 | 문항당 배점 | 파트 총점 |",
      "|---|---|---|---|---|---|---|",
      "| P1 | 어휘·어법 | 객관식 | 1~9 | 9 | 3점 | 27점 |",
      "| P2 | 독해 | 객관식 | 10~27 | 18 | 3점 (★1문항 4점) | 55점 |",
      "| 합계 | | | | 33 | | 100점 |",
      "",
      "**1. 어휘 — 정의 [3점]**  ",
      ...
      "핵심  astronomer = 천문학자  ",
      "오답 체크  A. farmer(농부)·C. sailor(선원)·D. painter(화가)·E. writer(작가)는 '별 연구'와 무관한 매력적인 오답이에요.",
    ].join("\n"),
  },
  {
    id: "worksheet",
    ...
    systemPrompt: [
      ...
      "",
      "Output format (STRICT — the renderer parses this exactly; keep it identical on every run):",
      "- Begin with a one-line title (subject + the specific skill/topic).",
      "- Then ONE '> ' blockquote line summarizing the key concept/rule (it renders as a 개념 박스).",
      "- Number problems continuously ('N. 발문'), ordered easier → harder. If a problem has options, put each on its OWN line labelled 'A) '…'E) '.",
      "- Leave two blank lines after each problem as working space.",
      "- End with a section headed exactly '## 정답 및 해설'. For each problem give 'N. 정답 …', a '핵심' line with the key point, and (for choice problems) an '오답 체크' line — in a warm, friendly teacher voice (따뜻한 '~요' 어투).",
      "",
      "Quality bar:",
    ...
    referenceExample: [
      "중2 영어 — 비교급 만들기 연습",
      "",
      "1. 다음 형용사의 비교급을 쓰시오: tall → ______",
      "",
      "",
      '2. 빈칸에 알맞은 말을 고르시오. "This box is ____ than that one."',
      "A) heavy  B) heavier  C) heaviest  D) more heavy",
      "",
      "",
      "## 정답 및 해설",
      "1. 정답 taller",
      "핵심  1음절 형용사는 보통 -er을 붙여요.",
      "2. 정답 B",
      "핵심  비교 대상이 둘이면 비교급(heavier)을 써요.",
      "오답 체크  C(heaviest)는 셋 이상일 때, D(more heavy)는 짧은 형용사엔 쓰지 않아요.",
    ].join("\n"),
  },
  {
    id: "quiz",
    ...
    systemPrompt: [
      ...
      "",
      "Output format (STRICT — the renderer parses this exactly; keep it identical on every run):",
      "- Number each question ('N. 발문'). For multiple choice, put each option on its OWN line labelled 'A) ' 'B) ' 'C) ' 'D) ' (up to 'E) '), with exactly one clearly correct answer.",
      "- Default to 5 multiple-choice questions unless the request specifies otherwise.",
      "- End with a section headed exactly '## 정답 및 해설'. For each question give 'N. 정답 X', a '핵심' line, and (for choice questions) an '오답 체크' line — in a warm, friendly teacher voice (따뜻한 '~요' 어투).",
      "",
      "Quality bar:",
    ...
    referenceExample: [
      "1. 광합성이 주로 일어나는 식물 기관은?",
      "A) 뿌리  B) 잎  C) 줄기  D) 꽃",
      "2. 광합성에 필요하지 않은 것은?",
      "A) 빛  B) 이산화탄소  C) 물  D) 질소",
      "",
      "## 정답 및 해설",
      "1. 정답 B",
      "핵심  잎의 엽록체에서 광합성이 일어나요.",
      "오답 체크  뿌리·줄기·꽃에는 엽록체가 거의 없어요.",
      "2. 정답 D",
      "핵심  광합성 3요소는 빛·이산화탄소·물이에요.",
      "오답 체크  질소는 광합성이 아니라 단백질 합성에 쓰여요.",
    ].join("\n"),
  },
  {
    id: "vocabulary",
  ...
  },
  {
    ...
    referenceExample: [
      ...
      "- 두 번째 요점 한 가지",
      "- 세 번째 요점 한 가지",
      "> Speaker notes: 이 슬라이드에서 전할 한두 문장 — 슬라이드에 없는 예시나 전환 멘트.",
      "",
      "## Slide 3: 마무리 · Q&A",
      "- 핵심 한 줄 요약",
      "- 질문 받기",
      "> Speaker notes: 감사 인사와 함께 자연스럽게 질문을 유도해요.",
    ].join("\n"),
  },
  {
    id: "study-notes",
  ...
  },
  {
    ...
    referenceExample: [
      "## 수업 개요",
      "- 과목: (과목) · 대상: 중학교 1학년 · 차시: 1/4 · 시간: 45분",
      "- 학습목표: 1) …을 설명할 수 있다. 2) …을 직접 만들 수 있다.",
      "",
      "준비물: 교과서, 활동지, 학습용 PPT",
      "",
      "| 단계 | 교사 활동 | 학생 활동 | 시간(분) | 자료·유의점 |",
      "|---|---|---|---|---|",
      "| 도입 | 전시 학습 확인, 동기 유발 질문 | 질문에 답하기 | 5 | PPT |",
      "| 전개 | 핵심 개념 설명, 활동 안내 | 짝 활동 수행·발표 | 30 | 활동지 |",
      "| 정리 | 핵심 정리, 차시 예고 | 핵심 한 줄 말하기 | 10 | — |",
      "",
      "## 평가",
      "- 활동지 완성도와 발표로 학습목표 달성을 확인한다.",
      "",
      "## 지도상 유의점",
      "- 모둠 활동 시간이 부족하지 않도록 모둠 수를 조절한다.",
    ].join("\n"),
  },
  {
    id: "resume",
  ...
  },
  {
    ...
    systemPrompt: [
      ...
      "",
      "MACHINE-READABLE WORKBOOK (STRICT — the app builds a real .xlsx from this):",
      "At the very END of your answer, append exactly one fenced block labelled xlsx containing ONLY valid JSON:",
      "```xlsx",
      '{"sheets":[{"name":"예시 데이터","rows":[["날짜","지역","금액"],["2025-01-05","서울",100],["2025-01-18","부산",200]],"formulas":[{"cell":"E2","f":"SUMIFS(C2:C10,B2:B10,\\"서울\\")","label":"서울 합계"}]}]}',
      "```",
      "- rows: a small sample table (headers + 3–8 rows) matching the layout you assumed; numbers as JSON numbers.",
      "- formulas: EVERY formula from your answer, placed in a sensible empty cell; 'f' WITHOUT the leading '='; optional 'label' names the cell to its left.",
      "- The JSON must parse as-is: double quotes only, no comments, no trailing commas, nothing else inside the fence.",
    ].join("\n"),
    referenceExample: [
      "목표: 월별 매출 합계를 구합니다. (데이터: A열 날짜, B열 금액, 1행 머리글)",
      "",
      "1. 1월 합계 셀(D2)에 입력:",
      '`=SUMIFS(B:B, A:A, ">="&DATE(2025,1,1), A:A, "<"&DATE(2025,2,1))`',
      "- `B:B` = 합계 범위(금액), `A:A` = 조건 범위(날짜)",
      "- 1월 1일 이상 ~ 2월 1일 미만 조건으로 1월 금액만 더해요.",
      "",
      "2. 엑셀 vs 구글 시트: 인수 구분자는 둘 다 ','를 쓰면 돼요(엑셀 한국어판도 보통 ',').",
      "",
      "```xlsx",
      '{"sheets":[{"name":"예시 데이터","rows":[["날짜","금액"],["2025-01-05",100],["2025-02-03",300]],"formulas":[{"cell":"D2","f":"SUMIFS(B2:B3,A2:A3,\\"2025-01-05\\")","label":"1월 합계(예시)"}]}]}',
      "```",
    ].join("\n"),
  },
];
