...
Each task below is **one self-contained block**. Copy a block → paste into Claude
Code (in this repo) → run. Order suggestion at the bottom. (Production system /
review prompts for the homepage SDK are in `docs/prompt-backlog.md` and
`docs/anthropic-sdk-prompts.md`.)
